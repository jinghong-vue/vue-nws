...
export default {
  ...
  methods: {
    ...
    deleteSelectionsProject() {
      ...
      for (let i = 0; i < selections.length; i++) {
        ...
        for (let j = 0; j < selectprojectEvery.items.length; j++) {
          // selectprojectEvery["items"][j]] ：每一个项目行中的项目数组中的每一个
          if (!bedoptdatas[selectprojectEvery["items"][j]]) {
            for (let k = 0; k < this.scheduleinfos.bed.length; k++) {
              if (
                this.scheduleinfos.bed[k]["number"] ==
                selectprojectEvery["items"][j]
              ) {
                bedoptdatas[selectprojectEvery["items"][j]] =
                  this.scheduleinfos.bed[k]["items"];
                break;
              }
            }
          }
          if (
            bedoptdatas[selectprojectEvery["items"][j]].indexOf(
              selectprojectEvery.number
            ) >= 0
          ) {
      ...
      this.$confirm("确定要删除所选的行吗", "删除提示")
        .then(() => {
          for (let i = 0; i < selections.length; i++) {
            ...
            // })
            // .catch(() => {});
            Promise.all([this.addbedtoproject(element)]).then((res) => {
              console.log(res);
              this.showbedtoproject();
            });
            // this.addbedtoproject(element).then((res)=>{
            //   console.log(res);
            // })
            // .catch((error)=>{
            //   console.log(error);
            // });
            // this.scheduleinfos.bed = this.scheduleinfos.bed.filter(item => {
            //   return item.number != element
            // })
            // this.reformatdata()
          }
          this.showbedtoproject();
    ...
    //点击dialog增加项目
    openDialog(index, data, focusindex) {
    ...
    //获取项目
    getProjects(count = -1, page = 1, areaid = "") {
    ...
    deleteRowOrArray(index, rows, itemindex = -1, type = "bed") {
      ...
      this.$confirm(deleteSingleitem, "删除提示")
        .then(() => {
          ...
          let delreqsts = [];
          for (const bednum in delbeds) {
            if (Object.hasOwnProperty.call(delbeds, bednum)) {
              delreqsts.push(_this.addbedtoproject(bednum, delbeds[bednum]));
            }
          }
          Promise.all(delreqsts).then((res) => {
            console.log("delbeds all: ", res);
      ...
      if (type === "bed") {
        ...
        for (let bi = 0; bi < this.scheduleinfos.bed.length; bi++) {
          ...
          for (let pi = 0; pi < bedelement["items"].length; pi++) {
            ...
            if (
              projectHasBeds[projectelement].indexOf(bedelement["number"]) < 0
            ) {
        ...
        for (let bi = 0; bi < this.scheduleinfos.project.length; bi++) {
          ...
          for (let pi = 0; pi < projectelement["items"].length; pi++) {
            ...
            if (
              projectHasBeds[bedelement].indexOf(projectelement["number"]) < 0
            ) {
